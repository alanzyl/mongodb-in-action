#!/bin/bash
export MONGO_DIR=/storage/mongodbexport
NUM_LOOPS=16
configs=(
    mmapv1.conf
...
for config in "${configs[@]}"; do
    ...
    T="$(date +%s)"
    /usr/bin/mongod --config configs/$config &

    # wait for mongo to start
    while [ 1 ]; do
        /usr/bin/mongostat -n 1 > /dev/null 2>&1
        if [ "$?" -eq 0 ]; then
            break
        fi
        sleep 2
    ...
    for l in $(seq 1 $NUM_LOOPS); do
        ...
        /usr/bin/mongo benchmark --quiet insert.js >/dev/null 2>&1
    done
    T="$(($(date +%s)-T))"
    echo
    ...
    T="$(date +%s)"
    /usr/bin/mongo admin --quiet --eval "db.shutdownServer({force: true})" >/dev/null 2>&1
    while [ 1 ]; do
